...
const props = defineProps<{ book: ShelfBook; pulled?: boolean }>()
defineEmits<{ open: [] }>()

const palette = computed(() => bookPalette(props.book.color))
const size = computed(() => bookSize(props.book))
// Alternate spine designs so the shelf doesn't look cloned
const style = computed(() => Math.floor(seeded(props.book.id, 3) * 3))

const vars = computed(() => ({
  "--w": `${size.value.width}px`,
  "--h": `${size.value.height}px`,
  "--d": `${size.value.depth}px`,
...
<template>
  <button
    ...
    @click="$emit('open')"
  >
    <span class="face spine" :data-style="style">
      <span class="band top" />
      <span class="title font-serif">{{ book.name }}</span>
      <span class="band bottom" />
      <UIcon name="i-lucide-chef-hat" class="emblem" />
    </span>
    <span class="face cover">
      <span class="cover-title font-serif">{{ book.name }}</span>
    </span>
    <span class="face back" />
    <span class="face pages-top" />
  </button>
</template>
